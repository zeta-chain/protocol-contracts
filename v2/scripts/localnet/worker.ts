...
const deployProtocolContracts = async (deployer: Signer, fungibleModuleSigner: Signer) => {
  ...
  const gatewayEVMInitdata = gatewayEVMInterface.encodeFunctionData(gatewayEVMInitFragment as ethers.FunctionFragment, [await deployer.getAddress(), testEVMZeta.target]);
  ...
  const zetaConnectorFactory = new ethers.ContractFactory(ZetaConnectorNonNative.abi, ZetaConnectorNonNative.bytecode, deployer);
  ...
  const systemContract = await systemContractFactory.deploy(ethers.ZeroAddress, ethers.ZeroAddress, ethers.ZeroAddress, deployOpts);
  ...
  const gatewayZEVMInitData = gatewayEVMInterface.encodeFunctionData(gatewayZEVMInitFragment as ethers.FunctionFragment, [wzeta.target]);
...
const deployTestContracts = async (
  ...
  await (token as any).connect(deployer).transfer(protocolContracts.custody.target, ethers.parseEther("500"), deployOpts);
  ...
  const zrc20 = await zrc20Factory.connect(fungibleModuleSigner).deploy(
    "TOKEN",
    "TKN",
    18,
    1,
  ...
  await (zrc20 as any).connect(fungibleModuleSigner).deposit(await deployer.getAddress(), ethers.parseEther("100"), deployOpts);
  await (zrc20 as any).connect(deployer).approve(protocolContracts.gatewayZEVM.target, ethers.parseEther("100"), deployOpts);

  // Include abi of gatewayZEVM events, so hardhat can decode them automatically
  const senderABI = [
    ...SenderZEVM.abi,
    ...GatewayZEVM.abi.filter((f) => f.type === "event"),
  ];
...
const startWorker = async () => {
  ...
  const wallet = new NonceManager(ethers.Wallet.fromPhrase(mnemonic, provider))

  const protocolContracts = await deployProtocolContracts(wallet, fungibleModuleSigner);
  const testContracts = await deployTestContracts(protocolContracts, wallet, fungibleModuleSigner);
  ...
  protocolContracts.gatewayZEVM.on("Call", async (...args: Array<any>) => {
    ...
    const executeTx = await protocolContracts.gatewayEVM.execute(receiver, message, { value: 0 });
  ...
  protocolContracts.gatewayEVM.on("Call", async (...args: Array<any>) => {
    ...
    const executeTx = await (protocolContracts.gatewayZEVM as any).connect(fungibleModuleSigner).execute(
