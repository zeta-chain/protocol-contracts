...
describe("GatewayZEVM inbound", function () {
  ...
  beforeEach(async () => {
    ...
    it("should withdraw zrc20 and emit event", async function () {
        const tx = await gateway.connect(owner).withdraw(ethers.utils.arrayify(addrs[0].address), parseEther("1"), ZRC20Contract.address);
        ...
        await expect(tx).to.emit(gateway, "Withdrawal")
            .withArgs(ethers.utils.getAddress(owner.address), addrs[0].address.toLowerCase(), parseEther("1"), 0, await ZRC20Contract.PROTOCOL_FLAT_FEE(), "0x");
    }); 
    ...
    it("should withdraw zrc20 and call and emit event with message", async function () {
        ...
        const tx = await gateway.connect(owner).withdrawAndCall(ethers.utils.arrayify(addrs[0].address), parseEther("1"), ZRC20Contract.address, message);
    
        const balanceOfAfterWithdrawal = await ZRC20Contract.balanceOf(owner.address) as BigNumber;
        expect(balanceOfAfterWithdrawal).to.equal(parseEther("99"));
    
        await expect(tx).to.emit(gateway, "Withdrawal")
            .withArgs(ethers.utils.getAddress(owner.address), addrs[0].address.toLowerCase(), parseEther("1"), 0, await ZRC20Contract.PROTOCOL_FLAT_FEE(), message);
        });
    ...
    it("should call and emit event without asset transfer", async function() {
        ...
        await expect(tx).to.emit(gateway, "Call")
