...
// IERC20CustodySupportsLegacyUpdatedIterator is returned from FilterSupportsLegacyUpdated and is used to iterate over the raw logs and unpacked data for SupportsLegacyUpdated events raised by the IERC20Custody contract.
type IERC20CustodySupportsLegacyUpdatedIterator struct {
	Event *IERC20CustodySupportsLegacyUpdated // Event containing the contract specifics and raw log
...
// are any more events found. In case of a retrieval or parsing error, false is
// returned and Error() can be queried for the exact failure.
func (it *IERC20CustodySupportsLegacyUpdatedIterator) Next() bool {
	// If the iterator failed, stop iterating
	if it.fail != nil {
		return false
	}
	...
	if it.done {
		select {
		case log := <-it.logs:
			it.Event = new(IERC20CustodySupportsLegacyUpdated)
			if err := it.contract.UnpackLog(it.Event, it.event, log); err != nil {
				it.fail = err
				return false
			}
...
// Error returns any retrieval or parsing error occurred during filtering.
func (it *IERC20CustodySupportsLegacyUpdatedIterator) Error() error {
	return it.fail
}

// Close terminates the iteration process, releasing any pending underlying
// resources.
func (it *IERC20CustodySupportsLegacyUpdatedIterator) Close() error {
	it.sub.Unsubscribe()
	return nil
}

// IERC20CustodySupportsLegacyUpdated represents a SupportsLegacyUpdated event raised by the IERC20Custody contract.
type IERC20CustodySupportsLegacyUpdated struct {
...
func (_IERC20Custody *IERC20CustodyFilterer) FilterSupportsLegacyUpdated(opts *bind.FilterOpts) (*IERC20CustodySupportsLegacyUpdatedIterator, error) {

	logs, sub, err := _IERC20Custody.contract.FilterLogs(opts, "SupportsLegacyUpdated")
	if err != nil {
		return nil, err
	}
	return &IERC20CustodySupportsLegacyUpdatedIterator{contract: _IERC20Custody.contract, event: "SupportsLegacyUpdated", logs: logs, sub: sub}, nil
}
...
//
// Solidity: event SupportsLegacyUpdated(bool newSupportsLegacy)
func (_IERC20Custody *IERC20CustodyFilterer) WatchSupportsLegacyUpdated(opts *bind.WatchOpts, sink chan<- *IERC20CustodySupportsLegacyUpdated) (event.Subscription, error) {

	logs, sub, err := _IERC20Custody.contract.WatchLogs(opts, "SupportsLegacyUpdated")
	if err != nil {
		return nil, err
	}
	return event.NewSubscription(func(quit <-chan struct{}) error {
		...
		for {
			...
			case log := <-logs:
				// New log arrived, parse the event and forward to the user
				event := new(IERC20CustodySupportsLegacyUpdated)
				if err := _IERC20Custody.contract.UnpackLog(event, "SupportsLegacyUpdated", log); err != nil {
					return err
				}
				event.Raw = log
...
//
// Solidity: event SupportsLegacyUpdated(bool newSupportsLegacy)
func (_IERC20Custody *IERC20CustodyFilterer) ParseSupportsLegacyUpdated(log types.Log) (*IERC20CustodySupportsLegacyUpdated, error) {
	event := new(IERC20CustodySupportsLegacyUpdated)
	if err := _IERC20Custody.contract.UnpackLog(event, "SupportsLegacyUpdated", log); err != nil {
		return nil, err
	}
